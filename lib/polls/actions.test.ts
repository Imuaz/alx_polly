/**
 * Comprehensive test suite for poll actions
 * Tests all poll-related functionality including creation, voting, deletion, and retrieval
 */
...
describe("poll actions", () => {
  describe("createPoll", () => {
    test("creates poll and options then redirects on success", async () => {
      ...
      // Act
      await expect(createPoll(fd)).rejects.toMatchObject({
      ...
      expect(mockRedirect).toHaveBeenCalledWith(
        "/polls?success=Poll created successfully!",
      );
    });
    ...
    test("handles database error during poll creation", async () => {
      ...
      await expect(createPoll(fd)).rejects.toThrow(
        "Failed to create poll: DB Error",
      );
    });
  ...
  describe("votePoll", () => {
    ...
    test("prevents double voting", async () => {
      ...
      const existingVoteSelect = makeSelectable({
        data: { id: "v1" },
        error: null,
      });
      ...
      await expect(votePoll("p1", ["opt1"])).rejects.toThrow(
    ...
    test("prevents voting on inactive polls", async () => {
      ...
      const existingVoteSelect = makeSelectable({ data: null, error: null });
      const pollSelect = makeSelectable({
        data: { allow_multiple_votes: false, status: "closed" },
        error: null,
      });
    ...
    test("prevents multiple votes on single-choice polls", async () => {
      ...
      await expect(votePoll("p1", ["opt1", "opt2"])).rejects.toThrow(
  ...
  describe("deletePoll", () => {
    test("deletes own poll and redirects", async () => {
      ...
      const ownerSelect = makeSelectable({
        data: { created_by: "u1" },
        error: null,
      });
      ...
      await expect(deletePoll("p1")).rejects.toMatchObject({
        digest: "NEXT_REDIRECT",
      });
      expect(mockRevalidatePath).toHaveBeenCalledWith("/polls");
      expect(mockRevalidatePath).toHaveBeenCalledWith("/dashboard");
      expect(mockRedirect).toHaveBeenCalledWith(
        "/dashboard?success=Poll deleted successfully!",
      );
    });
    ...
    test("prevents deleting others' polls", async () => {
      ...
      await expect(deletePoll("p1")).rejects.toThrow(
    ...
    test("handles poll not found error", async () => {
      ...
      await expect(deletePoll("nonexistent")).rejects.toThrow(
        "Failed to find poll: Poll not found",
      );
    });
    ...
    test("handles database error during deletion", async () => {
      ...
      await expect(deletePoll("p1")).rejects.toThrow(
        "Failed to delete poll: Delete failed",
      );
    });
  });
  ...
  describe("getPolls", () => {
    ...
    test("fetches all polls without filters", async () => {
      const mockQuery: any = {
        ...
        or: jest.fn(),
        order: jest.fn(() =>
          Promise.resolve({ data: mockPollsData, error: null }),
        ),
      };
    ...
    test("applies category filter", async () => {
      const mockQuery: any = {
        ...
        or: jest.fn(),
        order: jest.fn(() =>
          Promise.resolve({ data: [mockPollsData[0]], error: null }),
        ),
      };
    ...
    test("applies sorting by votes", async () => {
      ...
      expect(mockQuery.order).toHaveBeenCalledWith("total_votes", {
        ascending: true,
      });
    });
    ...
    test("applies sorting by ending date", async () => {
      ...
      expect(mockQuery.order).toHaveBeenCalledWith("ends_at", {
        ascending: false,
      });
    });
  ...
  describe("getPoll", () => {
    ...
    test("returns null on database error", async () => {
      const mockQuery = makeSelectable({
        data: null,
  ...
  describe("getUserPolls", () => {
    ...
    test("fetches polls for authenticated user", async () => {
      ...
      const mockQuery: any = {
        ...
        eq: jest.fn(),
        order: jest.fn(() =>
          Promise.resolve({ data: mockUserPollsData, error: null }),
        ),
      };
    ...
    test("handles empty poll options array", async () => {
      ...
      const mockQuery: any = {
        ...
        eq: jest.fn(),
        order: jest.fn(() =>
          Promise.resolve({ data: mockUserPollsDataNoOptions, error: null }),
        ),
      };
  ...
  describe("integration scenarios", () => {
    ...
    test("handles concurrent voting attempts", async () => {
      ...
      const existingVoteSelect2 = makeSelectable({
        data: { id: "v1" },
        error: null,
      });
      ...
      mockFrom.mockImplementation((table: string) => {
        if (table === "poll_votes") {
          ...
          return {
            select: jest.fn(() =>
              callCount === 1 ? existingVoteSelect1 : existingVoteSelect2,
            ),
            insert: voteInsert1,
          };
        }
      ...
      await expect(votePoll("p1", ["opt2"])).rejects.toThrow(
        "You have already voted on this poll",
      );
    });
  });
